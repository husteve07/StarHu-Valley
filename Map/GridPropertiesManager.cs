...
public class GridPropertiesManager : SingletonMonobehaviour<GridPropertiesManager>, ISaveable
{
    ...
    public void DisplayPlantedCrop(GridPropertyDetails gridPropertyDetails)
    {
        if (gridPropertyDetails.seedItemCode > -1)
        {
            ...
            gridPropertyDetails.gridPlantNeedWaterToGrow = cropDetails.needWaterToGrow;
            ...
            if (GameObjectSave.sceneData.TryGetValue(so_GridProperties.sceneName.ToString(), out SceneSave sceneSave))
            {
                if (sceneSave.gridPropertyDetailsDictionary != null)
                {
                    for (int i = sceneSave.gridPropertyDetailsDictionary.Count - 1; i >= 0; i--)
                    {
                        ...
                        {
                            if (gridPropertyDetails.gridPlantNeedWaterToGrow == false)
                            {
                                gridPropertyDetails.growthDays += 1;
                            }

                            else if (gridPropertyDetails.gridPlantNeedWaterToGrow == true && gridPropertyDetails.daysSinceWatered > -1)
                            {
                                gridPropertyDetails.growthDays += 1;
                            }
                        }
